...
class RedirectGenerator(implicit p: Parameters) extends XSModule
  ...
  // io.redirectPcRead.valid := oldestRedirect.valid
...
//  val store_pc = io.memPredPcRead(s1_redirect_bits_reg.stFtqIdx, s1_redirect_bits_reg.stFtqOffset, s1_redirect_valid_reg)
