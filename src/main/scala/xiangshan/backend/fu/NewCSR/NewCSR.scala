  ...
  with IpIeAliasConnect
{
  ...
  pmaEntryMod.io.in.pmaAddr := pmaaddr.map(_.regOut.asInstanceOf[PMPAddrBundle])
  ...
  pmaaddr.zipWithIndex.foreach { case (mod, i) =>
    mod.w.wen   := wenLegalReg && (addr === (CSRs.pmaaddr0 + i).U)
    mod.w.wdata := pmaEntryMod.io.out.pmaAddrWData(i)
  }
