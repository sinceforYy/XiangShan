...
class PMAEntryHandleModule(implicit p: Parameters) extends PMAModule with PMAInit with PMAReadWrite {
  ...
  val pmaMask = Reg(Vec(p(PMParameKey).NumPMA, UInt(PMPAddrBits.W)))

  pmaMask.zip(pmaInit).foreach { case (pmaMask, init) =>
    val addr = Wire(UInt(PMPAddrBits.W))
    addr := genAddr(init).U
    pmaMask := matchMask(init.a.U.asUInt(0), addr)
  }
      ...
      for (j <- cfgVec.indices) {
        ...
        when (!cfgOldTmp.L.asBool) {
          ...
          when (PMPCfgAField.isNa4OrNapot(cfgVec(j))) {
            pmaMask(8*i/2+j) := pmaEntry(8*i/2+j).matchMask(cfgVec(j), pmaEntry(8*i/2+j).addr.ADDRESS.asUInt)
          }
        }
      }
    }
  ...
  val pmaAddrW = Wire(Vec(p(PMParameKey).NumPMA, UInt(64.W)))
  val pmaAddrR = Wire(Vec(p(PMParameKey).NumPMA, UInt(64.W)))

  for (i <- 0 until p(PMParameKey).NumPMA) {
    pmaAddrW(i) := pmaEntry(i).addr.ADDRESS.asUInt
    pmaAddrR(i) := pmaEntry(i).addr.ADDRESS.asUInt
    // write pmaAddr
    when (wen && (addr === (CSRs.pmaaddr0 + i).U)) {
      if (i != (p(PMParameKey).NumPMA - 1)) {
        ...
        pmaAddrW(i) := Mux(!addrNextLocked, wdata, pmaEntry(i).addr.ADDRESS.asUInt)
      } else {
        ...
        pmaAddrW(i) := Mux(!addrLocked, wdata, pmaEntry(i).addr.ADDRESS.asUInt)
      }
    }
    // read pmaAddr
    when (ren && (addr === (CSRs.pmaaddr0 + i).U)) {
      pmaAddrR(i) := pmaEntry(i).readAddr(pmaEntry(i).cfg, pmaEntry(i).addr.ADDRESS.asUInt)
    }
  }

  io.out.pmaAddrWData := pmaAddrW
...
class PMAEntryHandleIOBundle(implicit p: Parameters) extends PMABundle {
  val in = Input(new Bundle {
    ...
    val pmaAddr = Vec(NumPMA, new PMPAddrBundle)
  })

  val out = Output(new Bundle {
    val pmaCfgWdata = UInt(PMXLEN.W)
    val pmaAddrRData = Vec(NumPMA, UInt(64.W))
    val pmaAddrWData = Vec(NumPMA, UInt(64.W))
  })
}
...
class PMAEntry(implicit p: Parameters) extends PMABundle with PMAReadWrite {
  ...
  def gen(cfg: PMACfgBundle, addr: PMPAddrBundle, mask: UInt) = {
